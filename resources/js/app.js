...
function visitorTracking(signal,config){if(!config.internal||!config.endpoint||navigator.globalPrivacyControl||navigator.doNotTrack==='1')return;const uuid=()=>crypto.randomUUID?.()||'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,c=>{const r=Math.random()*16|0;return(c==='x'?r:r&3|8).toString(16);});let visitorId,sessionId;try{visitorId=localStorage.getItem('studio-visitor-id')||uuid();localStorage.setItem('studio-visitor-id',visitorId);sessionId=sessionStorage.getItem('studio-session-id')||uuid();sessionStorage.setItem('studio-session-id',sessionId);}catch{return;}const pageviewId=uuid(),started=Date.now();let maxScroll=0,formStarted=false,sentFinal=false;const params=new URLSearchParams(location.search);const base={visitor_id:visitorId,session_id:sessionId,pageview_id:pageviewId,path:location.pathname,title:document.title,referrer:document.referrer?new URL(document.referrer).origin:'',locale:document.documentElement.lang,language:navigator.language,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,screen_width:screen.width,screen_height:screen.height,viewport_width:innerWidth,viewport_height:innerHeight,utm_source:params.get('utm_source'),utm_medium:params.get('utm_medium'),utm_campaign:params.get('utm_campaign'),utm_content:params.get('utm_content'),utm_term:params.get('utm_term')};const send=(payload,beacon=false)=>{const body=JSON.stringify({...base,...payload,_token:config.csrf});if(beacon&&navigator.sendBeacon){navigator.sendBeacon(config.endpoint,new Blob([body],{type:'application/json'}));return;}fetch(config.endpoint,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':config.csrf,'Accept':'application/json'},body,credentials:'same-origin',keepalive:true}).catch(()=>{});};const activity=(final=false)=>{if(final&&sentFinal)return;sentFinal=sentFinal||final;send({type:'activity',duration:Math.min(86400,Math.round((Date.now()-started)/1000)),scroll_depth:maxScroll},final);};const event=(name,label,target)=>send({type:'event',event_id:uuid(),event:name,label:(label||'').trim().slice(0,255),target_path:target||null});send({type:'pageview'});const onScroll=()=>{const available=Math.max(1,document.documentElement.scrollHeight-innerHeight);maxScroll=Math.max(maxScroll,Math.min(100,Math.round(scrollY/available*100)));};window.addEventListener('scroll',onScroll,{signal,passive:true});const interval=setInterval(()=>activity(),15000);signal.addEventListener('abort',()=>{clearInterval(interval);activity(true);},{once:true});document.addEventListener('visibilitychange',()=>{if(document.hidden)activity();},{signal});window.addEventListener('pagehide',()=>activity(true),{signal,once:true});document.addEventListener('click',e=>{const target=e.target.closest('a,button');if(!target)return;const href=target instanceof HTMLAnchorElement?target.href:'';const label=(target.getAttribute('aria-label')||target.textContent||'').trim();if(/^https?:\/\/(wa\.me|api\.whatsapp\.com)/i.test(href)||/^whatsapp:/i.test(href))event('whatsapp_click',label);else if(/^https?:/i.test(href)&&new URL(href).origin!==location.origin)event('outbound_click',label,new URL(href).pathname);else if(target instanceof HTMLAnchorElement&&target.hasAttribute('download'))event('download',label,new URL(href,location.href).pathname);else if(target.matches('.button,[data-cta]'))event('cta_click',label,target instanceof HTMLAnchorElement?new URL(href,location.href).pathname:null);},{signal});document.addEventListener('focusin',e=>{if(!formStarted&&e.target.matches('form input,form select,form textarea')){formStarted=true;event('form_start',e.target.closest('form')?.getAttribute('aria-label')||'form');}},{signal});document.addEventListener('submit',e=>event('form_submit',e.target.getAttribute('aria-label')||'form'),{signal});document.querySelectorAll('video').forEach(video=>{video.addEventListener('play',()=>{if(video.dataset.analyticsPlayed!=='1'){video.dataset.analyticsPlayed='1';event('video_play',video.getAttribute('aria-label')||document.title); }},{signal});video.addEventListener('ended',()=>event('video_complete',video.getAttribute('aria-label')||document.title),{signal});});}
function tracking(signal){const banner=document.getElementById('tracking-consent'),raw=document.getElementById('tracking-config');if(!banner||!raw)return;const config=JSON.parse(raw.textContent);let choice;try{choice=localStorage.getItem('studio-consent');}catch{}const enable=()=>{visitorTracking(signal,config);if(window.__studioExternalTracking)return;window.__studioExternalTracking=true;const script=(src)=>{const s=document.createElement('script');s.src=src;s.async=true;document.head.append(s);};if(/^G-[A-Z0-9]+$/.test(config.ga||'')){window.dataLayer=window.dataLayer||[];window.gtag=function(){window.dataLayer.push(arguments);};window.gtag('js',new Date());window.gtag('config',config.ga);script('https://www.googletagmanager.com/gtag/js?id='+config.ga);}if(/^[0-9]+$/.test(config.meta||'')){window.fbq=window.fbq||function(){window.fbq.callMethod?window.fbq.callMethod.apply(window.fbq,arguments):window.fbq.queue.push(arguments);};window.fbq.queue=[];window.fbq.loaded=true;window.fbq.version='2.0';script('https://connect.facebook.net/en_US/fbevents.js');window.fbq('init',config.meta);window.fbq('track','PageView');}if(/^[A-Za-z0-9_-]+$/.test(config.tiktok||'')){window.TiktokAnalyticsObject='ttq';window.ttq=window.ttq||[];window.ttq.methods=['page','track','identify','instances','debug','on','off','once','ready','alias','group','enableCookie','disableCookie'];window.ttq.methods.forEach(method=>window.ttq[method]=function(){window.ttq.push([method,...arguments]);});script('https://analytics.tiktok.com/i18n/pixel/events.js?sdkid='+config.tiktok+'&lib=ttq');window.ttq.page();}for(const [place,code]of [[document.head,config.head],[document.body,config.body]]){if(!code)continue;const temp=document.createElement('template');temp.innerHTML=code;for(const node of temp.content.childNodes){if(node.nodeName==='SCRIPT'){const s=document.createElement('script');for(const attr of node.attributes)s.setAttribute(attr.name,attr.value);s.textContent=node.textContent;place.append(s);}else place.append(node.cloneNode(true));}}};if(choice==='accept')enable();else if(choice!=='decline')banner.hidden=false;banner.querySelectorAll('[data-consent]').forEach(button=>button.addEventListener('click',()=>{choice=button.dataset.consent;try{localStorage.setItem('studio-consent',choice);}catch{}banner.hidden=true;if(choice==='accept')enable();},{signal}));}
